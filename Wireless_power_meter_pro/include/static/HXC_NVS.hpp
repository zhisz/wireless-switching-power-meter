 ...
 * @LastEditTime: 2025-02-18 18:11:16
 */
#ifndef HXC_NVS_HPP
#define HXC_NVS_HPP
#include "Arduino.h"
...
class NVS_Base {
    protected:
        ...
        static uint32_t _handle; // 共享的 NVS 句柄
};
...
class NVS_DATA<String> : public NVS_Base {
    ...
    esp_err_t save(){
        ...
        esp_err_t err = nvs_set_blob(_handle, key.c_str(),value.c_str(),value.length()); // 将数据写入NVS
        if(err){
            log_e("nvs_set_blob fail: %s %s", key.c_str(), nvs_error(err)); // 如果写入失败，记录错误信息
        }
        err = nvs_commit(_handle); // 提交写入操作
    ...
    String read(){
        ...
        err = nvs_get_blob(_handle, key.c_str(), (void*)arr, &datalen); // 读取数据
        if(err){
            log_e("KEY=%s NVS读取失败,使用默认值", key.c_str()); // 如果读取失败，记录错误信息
            return value; // 返回默认值
        }
